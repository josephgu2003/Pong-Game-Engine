...
void main()
{
    ...
    FragColor.rgb = brightness * color * clamp(texture(alphaMap, (TexCoords + vec2(0, 0.1* time)) * vec2(1.0, 10.0)).r - 0.4, 0, 1) * pow(max(dot(normalize(Normals), normalize(viewDir)), 0.0), 3);
   // FragColor.rgb = brightness * color * clamp(texture(alphaMap, (TexCoords + vec2(0.01 * time, 0)) * vec2(20.0, 1.0)).r - 0.3, 0, 1);
    ...
    float brightFactor = dot(FragColor.rgb, vec3(0.2126, 0.7152, 0.0722));
       ...
       if(brightFactor > 0.0)
           BrightColor = vec4(FragColor.rgb * 0, 1.0);
       else
