...
Sketch::Sketch(World* world_, Actor* actor_, float duration_, GLuint texture_) : Ability(world_, actor_, duration_) {
    for (int i = 0; i < 50; i++) {
        for (int j = 0; j < 50; j++) {
            quad.vertices.push_back((float)(-0.5f+i/49.0f));
            quad.vertices.push_back((float)(-0.5f+j/49.0f));
            quad.vertices.push_back(0.0f);
            quad.vertices.push_back((float)(0.0f+i/49.0f));
            quad.vertices.push_back((float)(0.0f+j/49.0f));
        }
    }
...
void Sketch::tick() {
    for (int i = 0; i < 50; i++) {
        for (int j = 0; j < 50; j++) {
            quad.vertices.at((50*i+j)*5+2) = 0.3*sin(4*(float)i/49.0f-2*duration);
